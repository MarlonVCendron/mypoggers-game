Shader "Custom/FillingWater" {
    ...
    SubShader {
        ...
        void surf(Input IN, inout SurfaceOutput o) {
            ...
            // Check the depth difference between the filling water and the depth mask
            float depth = LinearEyeDepth(tex2D(_CameraDepthTexture, IN.uv_MainTex).r);
            float maskDepth = tex2D(_CameraDepthTexture, IN.uv_MainTex).r;
            float depthDifference = depth - maskDepth;

            // Set the water's alpha based on the depth difference
